...
class StockMove(models.Model):
    ...
    requisition_id = fields.Many2one('item.requisition', 'Requisition')
...
class ItemRequisition(models.Model):
    ...
    destination = fields.Many2one('stock.location', string="Destination", domain="[('usage', 'in', ('internal','production'))]")
    ...
    order_line = fields.One2many('item.requisition.order.line', 'order_id', string='Order Lines', copy=True, track_visibility='always')
    ...
    def get_default_out_location_src_id(self):
        ...
        if self.picking_type_in_id:
            ...
            picking_type_in_id = self.env['stock.picking.type'].search([('code', '=', 'outgoing'),
                ('warehouse_id.company_id', 'in', [self.env.context.get('company_id', self.env.user.company_id.id), False])],
                limit=1).id
    ...
    picking_type_in_id = fields.Many2one('stock.picking.type', 'Operation Type ',default=get_default_out_picking_type)
    location_src_in_id = fields.Many2one('stock.location', 'Source Location', default=get_default_out_location_src_id,readonly=True)
    location_dest_in_id = fields.Many2one('stock.location', 'Destination Location', default=get_default_out_location_dest_id,readonly=True)
